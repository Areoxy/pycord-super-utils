...
bot = pycordSuperUtils.ManagerClient(
    "token", command_prefix="-", intents=discord.Intents.all()
)

InfractionManager = pycordSuperUtils.InfractionManager(bot)
BanManager = pycordSuperUtils.BanManager(bot)
KickManager = pycordSuperUtils.KickManager(bot)
MuteManager = pycordSuperUtils.MuteManager(bot)

bot.managers = [InfractionManager, BanManager, MuteManager]

InfractionManager.add_punishments(
    [
        pycordSuperUtils.Punishment(KickManager, punish_after=3),
        pycordSuperUtils.Punishment(MuteManager, punish_after=4),
        pycordSuperUtils.Punishment(BanManager, punish_after=5),
    ]
)


def make_removed_embeds(removed_infractions, member):
    return pycordSuperUtils.generate_embeds(
        [
...
@bot.event
async def on_ready():
    bot.database = pycordSuperUtils.DatabaseManager.connect(...)
...
async def mute(
    ...
    time_of_mute: pycordSuperUtils.TimeConvertor,
    reason: str = "No reason specified.",
):
    try:
        await MuteManager.mute(member, reason, time_of_mute)
    except pycordSuperUtils.AlreadyMuted:
...
async def ban(
    ctx,
    member: discord.Member,
    time_of_ban: pycordSuperUtils.TimeConvertor,
    reason: str = "No reason specified.",
):
...
async def get_before(
    ctx, member: discord.Member, from_time: pycordSuperUtils.TimeConvertor
):
